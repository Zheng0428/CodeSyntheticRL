...
def load_readme_data(input_path='data/input/gitrepo_5'):
    sample = 5
    ...
    for repo_dir in repo_dirs:
        ...
        folder_info = folder_to_string(repo_path, exclude_outer_readme=True)
...
def parse_your_response(prompt, question_id, repo_info):
    ...
    return {
        "id": f"readme_gen_{question_id}",
        "question_id": int(question_id),
        "task_id": '',
        "difficulty": '',
        "tags": ['readme', 'documentation', 'auto-generated'],
        "question": prompt,  # 使用prompt作为问题
        "answer": repo_info['readme_content'],  # 使用真实README作为答案
        "metadata": {
            "type": "readme_generation",
            "problem_description": "Generate comprehensive README documentation based on repository contents",
            "code": repo_info['folder_info'],
            "llm_generated": False,  # 因为答案是真实README，不是LLM生成的
...
def get_file_contents(root_dir, exclude_outer_readme=False, is_outer=True, outer_readme_name=None):
    """获取所有文件内容，只在最外层排除README，子目录中的README正常处理"""
    contents = []
    ...
    for dir in dirs:
        ...
        contents.extend(get_file_contents(dir_path, exclude_outer_readme, is_outer=False, outer_readme_name=outer_readme_name))
    
    return contents

def folder_to_string(root_dir, exclude_outer_readme=False):
    """将文件夹结构和内容转换为字符串，只排除最外层的README"""
    ...
    result.extend(structure)
    result.append("\n## 文件内容\n")
    
    # 添加文件内容（只在最外层排除README）
    file_contents = get_file_contents(root_dir, exclude_outer_readme=exclude_outer_readme, is_outer=True)
